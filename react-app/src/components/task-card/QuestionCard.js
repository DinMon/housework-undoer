...
const QuestionCard = ({ taskTitle, onChooseOption, className = '' }) => {
    ...
    function handleOptionClick(isDone) {
        onChooseOption(isDone)
        if (isDone) {
            setIsLoading(true)
        }
    }
    return (
        <Card colour='#FFE8DB' className={`detail-task-card ${className}`}>
            ...
            <div className='task-options'>
                <button className='task-option-btn cross-option' onClick={() => handleOptionClick(false)}>
                ...
                <button className='task-option-btn tick-option' onClick={() => handleOptionClick(true)}>
                    <div className='dash-circle-tick'>
                        <svg className={isLoading ? 'rotate' : ''} width="66" height="66" viewBox="0 0 66 66" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <circle cx="33" cy="33" r="32" stroke="#FF8933" strokeDasharray="5 5"/>
                        </svg>
                        <svg className={`tick ${isLoading ? 'pulse' : ''}`} width="36" height="27" viewBox="0 0 36 27" fill="none" xmlns="http://www.w3.org/2000/svg">
